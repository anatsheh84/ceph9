...
============================
OpenShift (ocp-external) using an EXTERNAL Ceph cluster (ceph9), with two views:

  TAB 1  Self-Service Provisioning  — log in with Ceph creds, then create &
         browse S3 buckets / NFS exports / CephFS subvolumes via the Ceph REST API.
  TAB 2  Consume Mounted Storage    — block (RBD PVC), file (CephFS PVC),
         object (OBC bucket): upload / list / download.

Provisioning uses the *logged-in user's* Ceph token, so Ceph enforces RBAC.
The mgr endpoint is auto-discovered (probing static node IPs) and per-endpoint
API versions are auto-negotiated, so it survives a lab rebuild.
"""
import io
import os
import re
...
class CephAPI:
    """Ceph mgr dashboard REST client: mgr discovery + per-endpoint API-version
    negotiation. Stateless w.r.t. auth — callers pass (base, token)."""

    @staticmethod
    def login(username, password):
        """Authenticate against any reachable mgr. Returns (base, token, is_admin)."""
        if not password:
    ...
    def _ok(cls, r, *codes):
        if r.status_code not in (codes or (200, 201)):
            ...
            if r.status_code == 403:
                raise CephAPIError("permission denied by Ceph RBAC (your user lacks this capability)")
...
def render_index(**extra):
    ...
    return render_template_string(
        ...
        ceph=ctx, active_tab=extra.pop("active_tab", "provision"), **extra)
...
def ceph_login():
    ...
    try:
        ...
        session["ceph_base"], session["ceph_token"] = base, token
        session["ceph_user"], session["ceph_admin"] = user, is_admin
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .tab{background:none;border:0;color:var(--muted);font-size:15px;font-weight:700;padding:14px 22px;cursor:pointer;
  ...
  select,input[type=file],input[type=text],input[type=password]{background:var(--panel-2);color:var(--text);
        border:1px solid var(--line);border-radius:6px;padding:10px 12px;font-size:14px;min-width:220px}
  ...
  button.ghost{background:#2b2b2b;color:#fff;border:1px solid var(--line);border-radius:6px;padding:8px 16px;font-weight:700;cursor:pointer}
  ...
  .dot{width:9px;height:9px;border-radius:50%}.dot.on{background:#3fb950}.dot.off{background:#777}
...
<div class="wrap">
  ...
  <section id="view-provision">
    <div class="card">
      ...
      <div class="hint">Self-service provisioning uses <b>your</b> Ceph credentials — Ceph enforces RBAC on what you can create.</div>
      {% if ceph %}
        <div class="status">
          <span class="dot on"></span>
          Connected as <b style="color:#fff">&nbsp;{{ ceph.user }}</b>
      ...
      {% else %}
        <form class="row" method="post" action="{{ url_for('ceph_login') }}">
          <div class="field"><label>Ceph username</label>
            <input type="text" name="username" value="admin" required></div>
          <div class="field"><label>Ceph password</label>
            <input type="password" name="password" placeholder="••••••••" required></div>
          <button class="go" type="submit">Connect &amp; verify</button>
        </form>
      {% endif %}
    </div>
      ...
      <div class="result"><h3>✓ Provisioned on ceph9 — connection details</h3>
        <div class="kv">
          {% for k, v in prov_result.items() if not k.startswith('_') and k != 'name' %}
            <div class="k">{{ k }}</div><div class="v">{{ v }}</div>{% endfor %}
        </div></div>
      {% endif %}
      <div class="card">
        <h2>Provision new storage on ceph9</h2>
        <div class="hint">Creates the resource via the Ceph mgr REST API and returns its connection details.</div>
        <form class="row" method="post" action="{{ url_for('provision') }}">
          <div class="field"><label>Type</label>
            <select name="ptype">
              <option value="s3">S3 bucket (RGW)</option>
              <option value="nfs">NFS export (nfslab)</option>
              <option value="cephfs">CephFS subvolume</option>
            </select></div>
          <div class="field"><label>Name</label>
            <input type="text" name="name" placeholder="my-bucket-1" pattern="[a-z0-9][a-z0-9-]{2,40}" required></div>
          <button class="go" type="submit">Provision +</button>
        </form>
      </div>
    {% endif %}
  </section>
  ...
  <section id="view-consume" class="hidden">
    <div class="card">
      <h2>Upload a file to mounted Ceph storage</h2>
      ...
      <form class="row" method="post" action="{{ url_for('upload') }}" enctype="multipart/form-data">
        ...
        <button class="go" type="submit">Upload &uarr;</button>
      </form>
    </div>
    <div class="grid">
...
<footer>Resources and files live on the external Ceph cluster (ceph9){% if node %} • node {{ node }}{% endif %}.</footer>
<script>
  function showTab(t){
    for(const n of ['provision','consume']){
